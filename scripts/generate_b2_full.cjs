// Direct CSV generator for the complete 500-word B2 batch
// Takes the approach of defining all words compactly and outputting CSV directly
const fs = require('fs');
...
function row(a,p,pl,tr,ex,pos,top,tag) {
  const w = nw(a);
  if (existing.has(w) || batchAdded.has(w)) return null;
  batchAdded.add(w);
  return [esc(''),'B2',esc(a),esc(p||''),esc(pl||''),esc(tr),esc(ex),esc(pos),esc(top),esc(tag),'B2_lesson_general'].join(',');
}

// Compact data format: [word, article, plural, translation, example, pos, topic, tag]
const entries = [];

function add(arr) {
  for (const [a,p,pl,tr,ex,pos,top,tag] of arr) {
    const r = row(a,p,pl,tr,ex,pos,top,tag);
    if (r) entries.push(r);
  }
}

// ================================================================
// 500 B2 German Words across 20 topics (25 each)
// Compact entries
// ================================================================
...
// Now add remaining 12 topics (300 more words)...
// Let's write what we have so far and think about the approach
