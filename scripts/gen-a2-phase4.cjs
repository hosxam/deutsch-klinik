...
const lcm   = require(path.join(HERE, "a2-mistakes.cjs"));
const lft   = require(path.join(HERE, "a2-forms.cjs"));
const lmd   = require(path.join(HERE, "a2-drills.cjs"));
const ltt   = require(path.join(HERE, "a2-tags.cjs"));
const gtl   = require(path.join(HERE, "a2-grammar-map.cjs"));
const kw    = require(path.join(HERE, "a2-kwmap.cjs"));
const wrRb  = require(path.join(HERE, "a2-writing-rubric.cjs"));
const spRb  = require(path.join(HERE, "a2-speaking-rubric.cjs"));
...
L("  l.commonMistakes = l.commonMistakes && l.commonMistakes.length ? l.commonMistakes : lcm[l.id] || [];");
L("  l.formsTable = l.formsTable && l.formsTable.length ? l.formsTable : lft[l.id] || [];");
L("  l.miniDrills = l.miniDrills && l.miniDrills.length ? l.miniDrills : lmd[l.id] || [];");
L("  l.trackTags = l.trackTags && l.trackTags.length ? l.trackTags : ltt[l.id] || [];");
...
L("let wC=0;d.writing.A2.forEach(x=>{if(!x.taughtInLessonId){x.taughtInLessonId=cMap(x);wC++;}if(!x.rubric){x.rubric=wrRb.map(r=>({criterion:r.criterion,description:r.description,points:r.points}));}if(!x.rubricKeys){x.rubricKeys=wrRb.map(r=>r.key);}});");
L('console.log("  Writing taughtInLessonId: "+wC+"/"+d.writing.A2.length+" rubric: "+d.writing.A2.filter(x=>x.rubric).length);');
L("let sC=0;d.speaking.A2.forEach(x=>{if(!x.taughtInLessonId){x.taughtInLessonId=cMap(x);sC++;}if(!x.rubric){x.rubric=spRb.map(r=>({criterion:r.criterion,description:r.description,points:r.points}));}if(!x.rubricKeys){x.rubricKeys=spRb.map(r=>r.key);}});");
...
L("});");
L("const totalLinks = a2l.reduce((s,l)=>s+l.linkedQuestionIds.length,0);");
L('console.log("  Total links: "+totalLinks);');
L("");
...
L('console.log("A2 Grammar: "+a2g.length+" items");');
L('console.log("A2 Reading: "+d.reading.A2.filter(x=>x.taughtInLessonId).length+"/"+d.reading.A2.length);');
L('console.log("A2 Listening: "+d.listening.A2.filter(x=>x.taughtInLessonId).length+"/"+d.listening.A2.length);');
L('console.log("A2 Writing: "+d.writing.A2.filter(x=>x.taughtInLessonId).length+"/"+d.writing.A2.length+" rubric: "+d.writing.A2.filter(x=>x.rubric).length);');
L('console.log("A2 Speaking: "+d.speaking.A2.filter(x=>x.taughtInLessonId).length+"/"+d.speaking.A2.length+" rubric: "+d.speaking.A2.filter(x=>x.rubric).length);');
L('console.log("\\nDone!");');
