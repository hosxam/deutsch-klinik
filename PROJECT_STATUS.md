...
| **AI Writing Correction** | Live via Cloudflare Worker endpoint at `deutsch-klinik-ai-correction.deutsch-klinik.workers.dev`, using Groq API (`llama-3.3-70b-versatile`, secret `GROQ_API_KEY`). Frontend reads `VITE_AI_CORRECTION_ENDPOINT` from `.env.local`. Shows score/10, rubric breakdown (5 keys), mistakes table, corrected version, improved version, flashcards. Falls back to copy-prompt when unconfigured. |
| **AI Speaking Feedback** | Same Worker, separated via `type` field (`"speaking"` vs `"writing"`). Frontend reads `VITE_AI_SPEAKING_ENDPOINT` (falls back to correction endpoint). Shows score/10, rubric breakdown (6 keys: task completion, grammar, vocabulary, structure, fluency, pronunciation risks), mistakes table, better phrases, corrected transcript, stronger answer, phrases to memorize. No audio sent to backend -- transcript only. Includes browser speech recognition (de-DE), local MediaRecorder audio (never uploaded), manual text entry. Falls back to copy-prompt when unconfigured. |
| **Speaking Prompts** | Timed speaking with preparation phase, tip section, useful phrases, AI feedback (transcript via Groq/Cloudflare Worker), local audio recording (never uploaded), browser speech recognition (de-DE), manual transcript entry, copy-prompt fallback |
